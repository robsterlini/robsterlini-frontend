...
function getFontFace(name, weight, style = "normal") {
  const source = `url('./fonts/${name}-${weight}.woff2') format('woff2')`;
  return new FontFace(name, source, { weight: weight, style: style });
}
...
function initFonts() {
  ...
  Promise.all([
    ...
    getFontFace("heading", 400).load(),
    getFontFace("heading", 400, "italic").load(),
